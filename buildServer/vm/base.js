...
class VMManager {
    constructor(large, region = '') {
        ...
        this.runBackgroundJobs = () => {
            ...
            const backgroundRedis = new ioredis_1.default(config_1.default.REDIS_URL);
            let vmBufferSize = 0;
            let vmBufferFlex = 0;
            if (this.isLarge) {
            ...
            const cleanupVMGroup = () => __awaiter(this, void 0, void 0, function* () {
                ...
                const allVMs = yield this.listVMs(this.getTag());
                const usedKeys = (yield this.redis.keys(`lock:${this.id}:*`)).map((key) => key.slice(`lock:${this.id}:`.length));
            ...
            const checkStaging = () => __awaiter(this, void 0, void 0, function* () {
                ...
                while (true) {
                    try {
                        ...
                        const id = yield backgroundRedis.brpoplpush(this.getRedisStagingKey(), this.getRedisStagingKey(), 0);
                        ...
                        if (ready) {
                            ...
                            if (retryCount % (25 * (1000 / checkStagingInterval)) === 0) {
                                const vm = yield this.getVM(id);
                                console.log('[CHECKSTAGING] attempt to poweron and attach to network');
                                this.powerOn(id);
